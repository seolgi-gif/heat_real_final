...
def font_setup():
    ...
    if nanum_gothic_files:
        ...
        print("NanumGothic font found and set.")
    else:
        # 없으면 경고 메시지 출력
...
# --- 2. 시나리오(재료) 정의 ---
# 2-1. 빠른 다층 시뮬레이션을 위한 단순화된 재료 물성
SCENARIOS = {
...
# 2-2. 히트맵 생성을 위한 상세 재료 물성 (첫 번째 코드에서 가져옴)
MATERIALS_DB_DETAILED = {
    ...
    "Aluminum": {"rho": 2700, "k": lambda T_K: 237 - 0.05 * (T_K - 273.15), "cp": lambda T_K: 900 + 0.45 * (T_K - 273.15)},
    "Steel": {"rho": 7850, "k": lambda T_K: 50.2 - 0.015 * (T_K - 273.15), "cp": lambda T_K: 490 + 0.2 * (T_K - 273.15)},
    "PCM": {
        "rho": 800,
        "k": lambda T_K, T_melt: np.where(T_K < T_melt, 0.22, 0.18),
        "cp": lambda T_K, T_melt: 2000 if T_K < T_melt else 2200,
        "T_melt_start_C": 140.0, "T_melt_end_C": 160.0, "L_h": 250000,
    }
}
# 이름 매핑 (Streamlit UI 이름 <-> 상세 DB 이름)
NAME_MAP = {
    '에어로겔': 'Aerogel',
    '세라믹 섬유': 'Ceramic_Fiber',
    'PCM (고체상태)': 'PCM',
    '강철 (Steel)': 'Steel',
    '알루미늄': 'Aluminum'
}
...
# 3-1. 빠른 다층 시뮬레이션 함수 (기존 Streamlit 코드)
@st.cache_data
...
# 3-2. 히트맵 생성을 위한 상세 시뮬레이션 함수 (첫 번째 코드에서 가져옴)
...
def run_detailed_single_material_simulation(material_name, total_thickness_mm):
    """주어진 단일 재료에 대해 2D 열 차폐 시뮬레이션을 수행하고 최종 온도 맵을 반환"""
    # 시뮬레이션 파라미터
    NX, NY = 51, 51
    LX = total_thickness_mm / 1000.0  # mm를 m로 변환
    LY = 0.1
    dx = LX / (NX - 1)
    dy = LY / (NY - 1)
    INITIAL_TEMP_C = 25.0
    TOTAL_SIM_TIME = 300 # 5분
    HEAT_SOURCE_TEMP_C = 800.0
    T_AMBIENT_C = 25.0
    h_conv = 10.0; epsilon = 0.8; SIGMA = 5.67e-8
    ...
    est_alpha = k_hot / (mat_props['rho'] * cp_hot)
    dt = 0.2 * (dx**2) / (2 * est_alpha)
    ...
    for step in range(1, n_steps + 1):
        ...
        # 오른쪽, 위, 아래 경계 조건 (대류 + 복사)
        q_out_right = h_conv*(T_K[:,-1] - T_ambient_K) + epsilon*SIGMA*(T_K[:,-1]**4 - T_ambient_K**4)
        H[:,-1] -= (q_out_right / dx) * dt
        q_out_top = h_conv*(T_K[-1,:] - T_ambient_K) + epsilon*SIGMA*(T_K[-1,:]**4 - T_ambient_K**4)
        H[-1,:] -= (q_out_top / dy) * dt
        q_out_bottom = h_conv*(T_K[0,:] - T_ambient_K) + epsilon*SIGMA*(T_K[0,:]**4 - T_ambient_K**4)
        H[0,:] -= (q_out_bottom / dy) * dt

        # 내부 전도
        k_x_interface = 0.5 * (k[:, 1:] + k[:, :-1])
        k_y_interface = 0.5 * (k[1:, :] + k[:-1, :])
        q_x = -k_x_interface * (T_C[:, 1:] - T_C[:, :-1]) / dx
        q_y = -k_y_interface * (T_C[1:, :] - T_C[:-1, :]) / dy
    ...
    for i, ui_name in enumerate(materials_for_heatmap_ui):
        with cols[i]:
            with st.spinner(f"'{ui_name}' 히트맵 생성 중..."):
                ...
                im = ax.imshow(final_map, cmap='inferno', vmin=initial_temp, vmax=heat_source_temp,
                               extent=[0, lx*100, 0, ly*100], origin='lower')
                ax.set_title(ui_name, fontproperties=font_prop)
                ax.set_xlabel('X-position (cm)', fontproperties=font_prop)
                if i == 0:
                    ax.set_ylabel('Y-position (cm)', fontproperties=font_prop)
